...
import { getLogoEquipo } from "../utils/sudamericanaLogos.jsx";

const API_BASE_URL = import.meta.env.VITE_API_URL;

export default function SimuladorSudamericana() {
  ...
  const cargarDatosJornada = async () => {
    try {
      ...
      const resPartidos = await fetch(
        `${API_BASE_URL}/api/sudamericana/jornadas/${jornadaSeleccionada}/partidos`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      const resPronosticos = await fetch(
        `${API_BASE_URL}/api/sudamericana/pronosticos/todos/jornada/${jornadaSeleccionada}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      const resRankingJornada = await fetch(
        `${API_BASE_URL}/api/sudamericana-rankings/jornada/${jornadaSeleccionada}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      const resRankingAcumulado = await fetch(
        `${API_BASE_URL}/api/sudamericana-rankings/acumulado/${jornadaSeleccionada}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>🎮 Simulador de Resultados - Copa Sudamericana</h2>
        <button className="btn btn-secondary" onClick={() => navigate("/sudamericana")}>
          ← Volver
        </button>
      </div>
