...
export default function AdminSudamericanaResultados() {
  ...
  useEffect(() => {
    const cargarJornadas = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/api/sudamericana/jornadas`, {
  ...
  const fetchPartidos = async (numero) => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_BASE_URL}/api/sudamericana/jornadas/${numero}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const guardarResultados = async () => {
    ...
    try {
      ...
      // Adaptar formato para el backend de Sudamericana
      ...
      const res = await fetch(`${API_BASE_URL}/api/sudamericana/jornadas/${jornadaSeleccionada}/resultados`, {
        method: "PATCH",
        headers: { 
  ...
  const calcularPuntajes = async () => {
    ...
    const mensaje = `¿Calcular puntajes de la jornada ${jornadaSeleccionada}?`;
    if (!confirm(mensaje)) return;
    
    try {
      setCalculando(true);
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_BASE_URL}/api/sudamericana-calcular/puntos`, {
        method: "POST",
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const generarPDFTestigo = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_BASE_URL}/api/sudamericana/pronosticos/generar-pdf/${jornadaSeleccionada}`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` }
      });
  ...
  const generarPDFCompleto = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const res = await fetch(`${API_BASE_URL}/api/sudamericana-ganadores-jornada/${jornadaSeleccionada}/pdf-final`, {
        method: "POST",
        headers: { "Authorization": `Bearer ${token}` }
      });
    ...
    try {
      ...
      for (let jornadaNum = 1; jornadaNum <= 6; jornadaNum++) {
        ...
        const responsePartidos = await fetch(
          `${API_BASE_URL}/api/sudamericana/jornadas/${jornadaNum}/partidos`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        // Guardar resultados
        await fetch(`${API_BASE_URL}/api/sudamericana/resultados`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json"
  ...
  const resetearGolesFaseGrupos = async () => {
    const confirmar = confirm(
      '⚠️ ¿RESETEAR todos los goles de la fase de grupos (Jornadas 1-6)?\n\n' +
      'Esto borrará todos los resultados de las 6 jornadas.\n\n' +
      '¿Estás seguro?'
    );
    ...
    try {
      ...
      // Obtener todos los partidos de jornadas 1-6
      let idsPartidos = [];
      for (let jornadaNum = 1; jornadaNum <= 6; jornadaNum++) {
        const responsePartidos = await fetch(
          `${API_BASE_URL}/api/sudamericana/jornadas/${jornadaNum}/partidos`,
        ...
        const partidosJornada = await responsePartidos.json();
        idsPartidos = [...idsPartidos, ...partidosJornada.map(p => p.id)];
      }

      // Resetear resultados (poner null en goles)
      const resultados = idsPartidos.map(id => ({
        id: id,
        golesLocal: null,
        golesVisita: null
      }));

      // Guardar resultados reseteados
      await fetch(`${API_BASE_URL}/api/sudamericana/resultados`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      alert('✅ Se resetearon todos los goles de las jornadas 1-6');
      ...
      console.error('Error al resetear goles:', error);
      alert('❌ Error al resetear goles');
    }
  };
  ...
  const toggleCierreJornada = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/sudamericana/jornadas/${jornada.id}/estado`, {
        method: "PATCH",
        headers: { 
          "Content-Type": "application/json",
          'Authorization': `Bearer ${token}`
  ...
  const toggleActivarJornada = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/sudamericana/jornadas/${jornadaSeleccionada}`, {
        method: "PATCH",
        headers: { 
          "Content-Type": "application/json",
          'Authorization': `Bearer ${token}`
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>📊 Resultados y Jornadas - Copa Sudamericana</h2>
        <div className="d-flex gap-2">
          ...
          <button 
            ...
            onClick={() => navigate('/admin/sudamericana/gestion')}
          >
            ← Volver
          </button>
        </div>
              ...
              <div className="d-flex gap-2 justify-content-center flex-wrap">
                {Number(jornadaSeleccionada) <= 6 && (
                  <>
                    ...
                    <button className="btn btn-outline-danger btn-lg" onClick={resetearGolesFaseGrupos}>
                      🗑️ Reset Fase Grupos
                    </button>
                  </>
                )}
