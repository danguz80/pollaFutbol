...
export default function ResumenJornadaSudamericana() {
  ...
  useEffect(() => {
    ...
    if (usuario.activo_sudamericana !== true) {
      alert("⚠️ No tienes acceso a Copa Sudamericana.");
      navigate("/");
      return;
    }
  }, []);
  ...
  const cargarResumen = async (numeroJornada) => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/sudamericana/pronosticos/resumen/jornada/${numeroJornada}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="container-fluid mt-4 px-4">
      <h2 className="text-center mb-4">📊 Resumen de Jornada - Copa Sudamericana</h2>
