...
export default function AdminLibertadoresResultados() {
  ...
  const generarPDFTestigo = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `Libertadores_Jornada_${jornadaSeleccionada}_${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  const generarPDFCompleto = async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `Resultados_Libertadores_Jornada_${jornadaSeleccionada}_${new Date().toISOString().split('T')[0]}.pdf`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
