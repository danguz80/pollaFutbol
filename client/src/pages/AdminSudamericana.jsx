...
export default function AdminSudamericana() {
  ...
  const procesarFaseGrupos = async () => {
    ...
    try {
      ...
      lineas.forEach(linea => {
        ...
        // Detectar jornada (acepta "Fecha" o "Jornada")
        const matchFecha = lineaTrim.match(/(?:Fecha|Jornada)\s*(\d+)/i);
      ...
      console.log('📤 Enviando equipos:', equiposArray);
      
      const respEquipos = await axios.post(
        `${API_URL}/api/sudamericana/equipos`,
        { equipos: equiposArray },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      console.log(`✅ ${equiposArray.length} equipos creados/actualizados`, respEquipos.data);
      ...
      for (let j = 1; j <= 6; j++) {
        if (jornadasPartidos[j].length > 0) {
          console.log(`📤 Enviando ${jornadasPartidos[j].length} partidos para Jornada ${j}`);
          
          const respPartidos = await axios.post(
            `${API_URL}/api/sudamericana/jornadas/${j}/partidos`,
            { partidos: jornadasPartidos[j] },
            { headers: { Authorization: `Bearer ${token}` } }
          );
          
          console.log(`✅ Jornada ${j} guardada:`, respPartidos.data);
  ...
  const generarSemiFinales = async () => {
    ...
    try {
      ...
      await axios.post(
        `${API_URL}/api/sudamericana/jornadas/10/partidos`,
        { partidos },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>🔴 Generador de Fixture - Copa Sudamericana</h2>
      ...
      <div className="card mb-4">
        <div className="card-header bg-success text-white">
          <h5 className="mb-0">⚽ Play-Offs (Jornada 7)</h5>
        </div>
        <div className="card-body">
          <p className="text-muted">
      ...
      <div className="card mb-4">
        <div className="card-header bg-warning text-dark">
          <h5 className="mb-0">🏆 Octavos de Final (Jornada 8)</h5>
        </div>
        <div className="card-body">
          <p className="text-muted">
            Ingresa los cruces de octavos de final (uno por línea). Formato: <code>Equipo Local vs Equipo Visitante</code>
            <br />
            Se generarán automáticamente los partidos IDA y VUELTA en la jornada 8.
          </p>
          <div className="mb-3">
            <label className="form-label">Cruces de Octavos de Final:</label>
            <textarea
              className="form-control font-monospace"
              rows="8"
      ...
      <div className="card mb-4">
        ...
        <div className="card-body">
          <p className="text-muted">
            Ingresa los cruces de cuartos de final (uno por línea). Formato: <code>Equipo Local vs Equipo Visitante</code>
            <br />
            Se generarán automáticamente los partidos IDA y VUELTA en la jornada 9.
          </p>
          <div className="mb-3">
            <label className="form-label">Cruces de Cuartos de Final:</label>
            <textarea
              className="form-control font-monospace"
              rows="4"
              placeholder="Ejemplo:&#10;Flamengo vs River Plate&#10;Boca Juniors vs Palmeiras&#10;..."
      ...
      <div className="alert alert-info">
        ...
        <ul className="mb-0">
          ...
          <li>Los partidos se crean con bonus predeterminados (ajustables desde Resultados y Jornadas)</li>
        </ul>
      </div>
    </div>
  );
