...
const StarWithNumber = ({ number }) => (
  <span style={{
    ...
    <svg width={isMobile ? 28 : 38} height={isMobile ? 28 : 38} viewBox="0 0 20 20" fill="#28a745" style={{ display: 'block' }}>
      <polygon points="10,1 12.5,7.5 19,7.5 13.5,12 15.5,18.5 10,14.5 4.5,18.5 6.5,12 1,7.5 7.5,7.5" />
    </svg>
    <span style={{
      position: 'absolute',
...
export default function GanadoresJornadaSudamericana() {
  ...
  const [ultimaJornadaCerrada, setUltimaJornadaCerrada] = useState(false);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const acumuladoRes = await fetch(`${API_BASE_URL}/api/sudamericana-ganadores-jornada/acumulado`, { headers });
        ...
        // Filtrar usuarios sin admin (no filtrar por activo_sudamericana para que siempre aparezcan)
        const jugadoresActivos = usuariosData
          .filter(u => u.rol !== 'admin')
          .map(u => u.nombre);
        setJugadores(jugadoresActivos);

        // Obtener ganadores de cada jornada (cerrada o no, si hay ganadores calculados)
        const ganadoresPorJornadaTemp = {};
        for (const jornada of jornadasData) {
          try {
            const ganadoresRes = await fetch(
              `${API_BASE_URL}/api/sudamericana-ganadores-jornada/${jornada.numero}`,
              { headers }
            );
  ...
  const tablaGanadores = (
    <div style={{ maxWidth: 500, margin: '0 auto', marginBottom: 40 }}>
      ...
      <table className="table table-bordered text-center">
        <thead>
          <tr style={{ background: '#20c997', color: '#fff', fontWeight: 'bold', fontSize: isMobile ? '0.9em' : '1.1em' }}>
  ...
  function getRankingGanadoresCellStyle(posicion) {
    if (posicion === 1) return { background: "#28a745", color: "white", fontWeight: "bold", fontSize: "1.15em", textAlign: "center" };
    if (posicion === 2) return { background: "#20c997", color: "white", fontWeight: "bold", fontSize: "1.15em", textAlign: "center" };
    if (posicion === 3) return { background: "#5cb85c", color: "white", fontWeight: "bold", fontSize: "1.15em", textAlign: "center" };
  ...
  return (
    <div className="container mt-4">
      <div className="text-center mb-4">
        <h1 className="display-5 fw-bold" style={{ color: '#28a745' }}>🟢 Copa Sudamericana 2026</h1>
      ...
      <div style={{ maxWidth: 470, margin: "0 auto" }}>
        <table className="table table-bordered text-center ganadores-jornada-tbl">
          <thead>
            <tr style={{ background: "#20c997", color: "#fff", fontWeight: "bold", fontSize: isMobile ? '1.13em' : '1.25em' }}>
              <th>Posición</th>
              <th>Participante</th>
