...
router.get('/equipos', async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT * FROM sudamericana_equipos 
...
router.post('/equipos', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    await pool.query('DELETE FROM sudamericana_equipos');
      ...
      for (const grupo of grupos) {
        for (let pos = 1; pos <= 4; pos++) {
          ...
          if (equipo && equipo.nombre) {
            await pool.query(`
              INSERT INTO sudamericana_equipos (nombre, grupo, posicion_grupo, api_id, pais)
              VALUES ($1, $2, $3, $4, $5)
...
router.get('/jornadas', async (req, res) => {
  try {
    let result = await pool.query(`
      SELECT * FROM sudamericana_jornadas 
      ORDER BY numero
    `);
    ...
    if (result.rows.length === 0) {
      ...
      for (const jornada of jornadas) {
        await pool.query(`
          INSERT INTO sudamericana_jornadas (numero, nombre, activa, cerrada)
          VALUES ($1, $2, $3, false)
...
router.get('/jornadas/:numero', async (req, res) => {
  try {
    ...
    const jornadaResult = await pool.query(`
      SELECT * FROM sudamericana_jornadas WHERE numero = $1
    `, [numero]);
...
router.patch('/jornadas/abrir-todas', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    await pool.query('UPDATE sudamericana_jornadas SET cerrada = false');
    const result = await pool.query('SELECT * FROM sudamericana_jornadas ORDER BY numero');
    res.json({ 
    ...
    if (fecha_cierre) {
      ...
      await pool.query(
        ...
        [
          ...
          `⏰ Fecha de cierre actualizada para Jornada ${jornada.numero} - Sudamericana: ${fechaFormateada}`,
          '⏰',
          `/sudamericana/jornada/${jornada.numero}`,
          jornada.numero
        ]
      );
      console.log(`✅ Notificación Sudamericana: fecha de cierre actualizada para jornada ${jornada.numero}`);
    }
    ...
    if (cerrada === true) {
      await pool.query(
        ...
        [
          ...
          'jornada',
          'jornada_cerrada',
          `🔒 La Jornada ${jornada.numero} de Sudamericana ha sido cerrada. Ya no se aceptan pronósticos.`,
          '🔒',
          `/sudamericana/jornada/${jornada.numero}`,
          jornada.numero
        ]
      );
      console.log(`✅ Notificación Sudamericana: jornada ${jornada.numero} cerrada`);
    }
...
router.post('/jornadas/:numero/partidos', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    if (req.body.partidos) {
      ...
      const jornadaResult = await pool.query(
        'SELECT id FROM sudamericana_jornadas WHERE numero = $1',
        [numero]
      );
      ...
      await pool.query('DELETE FROM sudamericana_partidos WHERE jornada_id = $1', [jornadaId]);
...
// Actualizar múltiples resultados de partidos (Admin)
router.post('/resultados', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    const { resultados } = req.body; // Array de {id, golesLocal, golesVisita}
    ...
    for (const resultado of resultados) {
      const { id, golesLocal, golesVisita } = resultado;
      
      if (id && golesLocal !== undefined && golesVisita !== undefined) {
        await pool.query(
          'UPDATE sudamericana_partidos SET goles_local = $1, goles_visita = $2 WHERE id = $3',
    ...
    res.json({ 
      mensaje: 'Resultados actualizados exitosamente', 
      partidosActualizados 
    });
  } catch (error) {
  ...
  try {
    ...
    if (jornadaNumero === 7 || jornadaNumero === 8 || jornadaNumero === 9 || jornadaNumero === 10) {
      ...
      const jornadasResult = await pool.query(
        'SELECT id, numero FROM sudamericana_jornadas WHERE numero IN ($1, $2) ORDER BY numero',
    ...
    if (jornadaNumero === 10) {
      const jornadaResult = await pool.query(
        'SELECT id FROM sudamericana_jornadas WHERE numero = 10'
      );
      ...
      if (resultadosIngresados > 0) {
        await pool.query(
          ...
          [
            ...
            `📊 Se han agregado ${resultadosIngresados} resultado(s) real(es) en la Jornada ${req.params.numero} - Sudamericana`,
            '📊',
            `/sudamericana/jornada/${req.params.numero}`,
            parseInt(req.params.numero)
          ]
        );
        console.log(`✅ Notificación Sudamericana: resultados agregados para jornada ${req.params.numero}`);
      }
    }
...
router.patch('/partidos/:id/bonus', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE sudamericana_partidos SET bonus = $1 WHERE id = $2 RETURNING *',
      [bonus, id]
    );
...
router.delete('/partidos/:id', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'DELETE FROM sudamericana_partidos WHERE id = $1 RETURNING *',
      [id]
    );
...
router.delete('/jornadas/:numero/partidos', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    await pool.query(
      'DELETE FROM sudamericana_pronosticos WHERE jornada_id = $1',
      [jornadaId]
    );
    
    // Luego borrar los partidos
    const result = await pool.query(
      'DELETE FROM sudamericana_partidos WHERE jornada_id = $1 RETURNING *',
      [jornadaId]
    );
...
router.post('/octavos', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    const deletePartidosResult = await pool.query(
      'DELETE FROM sudamericana_partidos WHERE jornada_id = $1', 
      [jornadaId]
    );
...
router.patch('/jornadas/:id/estado', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE sudamericana_jornadas SET cerrada = $1 WHERE id = $2 RETURNING *',
      [cerrada, id]
    );
...
router.post('/cuartos', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    let jornadaResult = await pool.query(
      'SELECT id FROM sudamericana_jornadas WHERE numero = 9'
    );
    
    if (jornadaResult.rows.length === 0) {
      jornadaResult = await pool.query(
        'INSERT INTO sudamericana_jornadas (numero, nombre, activa) VALUES (9, $1, false) RETURNING id',
...
router.post('/semifinales', verifyToken, authorizeRoles('admin'), async (req, res) => {
  try {
    ...
    if (jornadaResult.rows.length === 0) {
      jornadaResult = await pool.query(
        'INSERT INTO sudamericana_jornadas (numero, nombre, activa) VALUES (10, $1, false) RETURNING id',
    ...
    await pool.query(
      `DELETE FROM sudamericana_partidos 
       WHERE jornada_id = $1 
