...
export default function AdminLibertadoresGestion() {
  ...
  const verificarRespaldo = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/admin/verificar-respaldo-libertadores?temporada=${temporada}`,
  ...
  const cargarEstadisticas = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(
        `${API_URL}/api/admin/estadisticas-libertadores`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const crearRespaldo = async () => {
    if (!confirm(`¿Crear respaldo de ganadores de Copa Libertadores ${temporada} en Rankings Históricos?`)) {
      return;
    }
    ...
    try {
      ...
      const response = await axios.post(
        `${API_URL}/api/rankings-historicos/actualizar?temporada=${temporada}&competencia=libertadores`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const eliminarDatos = async () => {
    ...
    const confirmacion = window.prompt(
      `⚠️ ADVERTENCIA: Esta acción eliminará todos los datos de Copa Libertadores.\n\n` +
    ...
    try {
      ...
      const response = await axios.delete(
        `${API_URL}/api/admin/eliminar-datos-libertadores?temporada=${temporada}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="container mt-4">
      {/* Botonera Principal Libertadores */}
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h2>🔴 Administración Copa Libertadores</h2>
      ...
      {estadisticas && (
        <div className="card mb-4">
          <div className="card-header">
            <h5>📊 Datos Actuales de Libertadores</h5>
          </div>
          <div className="card-body">
            <div className="row">
        ...
        <div className="card-body">
          <div className="d-grid gap-3">
            ...
            <div>
              ...
              >
                {loadingEliminar ? '⏳ Eliminando...' : '🗑️ Eliminar Datos de Libertadores'}
              </button>
              <small className="text-muted d-block mt-2">
                Elimina todos los datos de Libertadores para preparar el nuevo año.
