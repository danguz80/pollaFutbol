        ...
        for (const jornada of jornadasData.filter(j => j.cerrada)) {
          try {
            const ganadoresRes = await fetch(
              `${API_BASE_URL}/api/libertadores-ganadores-jornada/${jornada.numero}`,
              { headers }
            );
  ...
  const tablaGanadores = (
    <div style={{ maxWidth: 500, margin: '0 auto', marginBottom: 40 }}>
      ...
      <table className="table table-bordered text-center">
        <thead>
          <tr style={{ background: '#ff5252', color: '#fff', fontWeight: 'bold', fontSize: isMobile ? '0.9em' : '1.1em' }}>
  ...
  return (
    <div className="container mt-4">
      <div className="text-center mb-4">
        <h1 className="display-5 fw-bold" style={{ color: '#d32f2f' }}>🔴 Copa Libertadores 2026</h1>
      ...
      <div style={{ maxWidth: 470, margin: "0 auto" }}>
        <table className="table table-bordered text-center ganadores-jornada-tbl">
          <thead>
            <tr style={{ background: "#ff5252", color: "#fff", fontWeight: "bold", fontSize: isMobile ? '1.13em' : '1.25em' }}>
              <th>Posición</th>
              <th>Participante</th>
